...
counts---; those QC metrics already give a good idea of technical issues
in certain samples.
...
for reference
(median: `r sprintf("%.1fe+06", 10^multiqc.assigned.M/1E6)`,
lower bound:
`r sprintf("%.1fe+06", 10^(multiqc.assigned.M-multiqc.assigned.MAD*3)/1E6)`,
upper bound:
`r sprintf("%.1fe+06", 10^(multiqc.assigned.M+multiqc.assigned.MAD*3)/1E6)`):

```{r multiQC_ggplot_assigned, echo=FALSE}
ggplot(multiqc, aes(Sample, featureCounts_Assigned)) +
  geom_point(aes(colour = Status, shape = Infection)) +
  geom_text_repel(
    aes(label = Time), subset(multiqc[idx.sc,], multiqc.assigned.outliers),
    alpha = 0.4
  ) +
  geom_hline(
...
Let us visualise outliers, along with blank and bulk samples
for reference
(median: `r sprintf("%.1f%%", multiqc.GC.M)`,
lower bound: `r sprintf("%.1f", multiqc.GC.M-multiqc.GC.MAD*3)`,
upper bound: `r sprintf("%.1f", multiqc.GC.M+multiqc.GC.MAD*3)`):

```{r multiQC_ggplot_GC, echo=FALSE}
ggplot(multiqc, aes(Sample, FastQC_percent_gc)) +
  geom_point(aes(colour = Status, shape = Infection)) +
  geom_text_repel(
    aes(label = Time), subset(multiqc[idx.sc,], multiqc.GC.outliers),
    alpha = 0.4
  ) +
  geom_hline(
...
for reference
(median: `r sprintf("%.1f%%", multiqc.aligned.M)`,
lower bound:
`r 10^(multiqc.assigned.M-multiqc.assigned.MAD*3)`,
upper bound:
`r sprintf("%.1fe+06", 10^(multiqc.assigned.M+multiqc.assigned.MAD*3)/1E6)`):
...
ggplot(multiqc, aes(Sample, Bowtie.2_overall_alignment_rate)) +
  geom_point(aes(colour = Status, shape = Infection)) +
  geom_text_repel(
    aes(label = Time), subset(multiqc[idx.sc,], multiqc.aligned.outliers),
    alpha = 0.4
  ) +
  geom_hline(
...
ggplot(multiqc, aes(Sample, featureCounts_percent_assigned)) +
  geom_point(aes(colour = Status, shape = Infection)) +
  geom_text_repel(
    aes(label = Time), subset(multiqc[idx.sc,], multiqc.assignedPerc.outliers),
    alpha = 0.4
  ) +
  geom_hline(
...
an earlier section [above](#featureCounts):

```{r scater_outliers_totalCounts}
multiqc.libsize <- log10(multiqc$featureCounts_Assigned[idx.sc])
names(multiqc.libsize) <- multiqc$Sample[idx.sc]
sce.sc.libsize <- sce.sc$log10_total_counts
names(sce.sc.libsize) <- sce.sc$Sample
multiqc.libsize <- multiqc.libsize[names(multiqc.libsize)]
all(multiqc.libsize == sce.sc.libsize)
```

As a result, this metric is identical to the *featureCounts_Assigned*
metric described [above](#featureCounts); and thereby would
...
ggplot(pData(sce.all), aes(Sample, pct_counts_top_50_features)) +
  ...
  geom_text_repel(
    aes(label = Time), subset(pData(sce.sc), scater.top50.outliers),
    alpha = 0.4) +
  geom_hline(
    yintercept = scater.top50.M + 3 * c(-1,1) * scater.top50.MAD,
    colour = "red", linetype = 2:1) +
...
ggplot(pData(sce.all), aes(Sample, pct_counts_top_100_features)) +
  geom_point(aes(colour = Time, shape = Infection)) +
  geom_text_repel(
    aes(label = Time), subset(pData(sce.sc), scater.top100.outliers),
    alpha = 0.4) +
  geom_hline(
    yintercept = scater.top100.M + 3 * c(-1,1) * scater.top100.MAD,
    colour = "red", linetype = 2:1) +
...
ggplot(pData(sce.all), aes(Sample, pct_counts_top_200_features)) +
  geom_point(aes(colour = Time, shape = Infection)) +
  geom_text_repel(
    aes(label = Time), subset(pData(sce.sc), scater.top200.outliers),
    alpha = 0.4) +
  geom_hline(
    yintercept = scater.top200.M + 3 * c(-1,1) * scater.top200.MAD,
    colour = "red", linetype = 2:1) +
...
ggplot(pData(sce.all), aes(Sample, total_features)) +
  geom_point(aes(colour = Status, shape = Infection)) +
  geom_text_repel(
    aes(label = Time), subset(pData(sce.sc), scater.features.outliers),
    alpha = 0.4
  ) +
  geom_hline(
...
ggplot(pData(sce.all), aes(Sample, pct_counts_feature_controls_ERCC)) +
  geom_point(aes(colour = Status, shape = Infection)) +
  geom_text_repel(
    aes(label = Time), subset(pData(sce.sc), scater.ERCC.outliers),
    alpha = 0.6
  ) +
  geom_hline(
...
ggplot(pData(sce.all), aes(Sample, pct_counts_feature_controls_MT)) +
  geom_point(aes(colour = Status, shape = Infection)) +
  geom_text_repel(
    aes(label = Time), subset(pData(sce.sc), scater.MT.outliers),
    alpha = 0.4
  ) +
  geom_hline(
...
hm <- Heatmap(
  ...
  split = sce.sc$Time[sample.order.supervised]
)
draw(hm + hm_row)
```
