...
col.status<-col.set3[c(3,5:6)];names(col.status)<-levels(sce.all$Status)[1:3]
...
plot.GC <- ggplot(pData(sce.all), aes(Sample, FastQC_percent_gc)) +
  ...
  geom_text_repel(
    aes(label = Time), pData(sce.sc)[outliers$GC,], alpha = 0.4) +
  geom_hline(yintercept = GC.M + 3 * c(1,-1) * GC.MAD, colour = "red") +
  geom_hline(yintercept = GC.M) + labs(y = "% GC (FastQC)") +
  scale_y_continuous(limits = c(0,100)) + sampleQCtheme
plot.GC
```
...
plot.aligned <-
  ...
  geom_point(aes(colour = Status, shape = Infection)) +
  geom_text_repel(
    aes(label = Time), pData(sce.sc)[outliers$AlignedPercent,], alpha = 0.4 ) +
  scale_y_continuous(limits = c(0,100)) + geom_hline(yintercept = align.M) +
  geom_hline(
    yintercept = align.M + 3*c(1,-1)*align.MAD, colour="red", linetype=2:1) +
  labs(y = "Overall alignment rate (HISAT2)") + sampleQCtheme
plot.aligned
```
...
plot.cAssigned <-
  ...
  geom_point(aes(colour = Status, shape = Infection)) +
  geom_text_repel(
    aes(label = Time), pData(sce.sc)[outliers$AssignedCount,], alpha = 0.4
  ) + geom_hline(
    yintercept = 10^(cAssigned.M + 3*c(1,-1)*cAssigned.MAD),
    colour = "red") + geom_hline(yintercept = 10^(cAssigned.M)) +
  labs(y = "Count assigned (featureCounts)") + sampleQCtheme
plot.cAssigned
```
...
plot.pAssigned <-
  ...
  geom_point(aes(colour = Status, shape = Infection)) +
  geom_text_repel(
    aes(label = Time), pData(sce.sc)[outliers$AssignedPercent,], alpha = 0.4
  ) + scale_y_continuous(limits = c(0,100)) + geom_hline(
    yintercept = (pAssigned.M + 3*c(1,-1)*pAssigned.MAD), colour = "red",
    linetype = 2:1) + geom_hline(yintercept = pAssigned.M) +
  labs(y = "% assigned (featureCounts)") + sampleQCtheme
plot.pAssigned
```
...
plot.top50 <-
  ...
  geom_point(aes(colour = Status, shape = Infection)) +
  geom_text_repel(
    aes(label = Time), pData(sce.sc)[outliers$top50,], alpha=0.4) +
  scale_y_continuous(limits = c(0,100)) + geom_hline(
    yintercept = top50.M + 3*c(-1,1)*top50.MAD, colour="red", linetype=2:1) + labs(y = "% counts (top 50 features)") +
  geom_hline(yintercept = top50.M) + sampleQCtheme
plot.top50
```
...
plot.top100 <-
  ...
  geom_point(aes(colour = Status, shape = Infection)) +
  geom_text_repel(
    aes(label = Time), pData(sce.sc)[outliers$top100,], alpha=0.4) +
  scale_y_continuous(limits = c(0,100)) + geom_hline(
    yintercept = top100.M + 3*c(-1,1)*top100.MAD,
    colour = "red", linetype = 2:1) + labs(y = "% counts (top 100 features)") +
  geom_hline(yintercept = top100.M) + sampleQCtheme
plot.top100
```
...
plot.top200 <-
  ...
  geom_point(aes(colour = Status, shape = Infection)) +
  geom_text_repel(
    aes(label = Time), pData(sce.sc)[outliers$top200,], alpha=0.4) +
  scale_y_continuous(limits = c(0,100)) + geom_hline(
    yintercept = top200.M + 3*c(-1,1)*top200.MAD,
    colour = "red", linetype = 2:1) + labs(y = "% counts (top 100 features)") +
  geom_hline(yintercept = top200.M) + sampleQCtheme
plot.top200
```
...
plot.features <-
  ...
  geom_point(aes(colour = Status, shape = Infection)) +
  geom_hline(yintercept = features.M) + geom_hline(
    yintercept = features.M + 2 * c(-1,1) * features.MAD, colour = "red",
    linetype = 1:2) +
  scale_y_continuous(limits=c(0,max(sce.all$total_features))) + geom_text_repel(
    aes(label = Time), pData(sce.sc)[outliers$FeaturesCount,], alpha = 0.4) +
  labs(y = "Count of detected features") + sampleQCtheme
plot.features
```
...
ggplot(pData(sce.all), aes(Sample, pct_counts_feature_controls_MT)) +
  geom_point(aes(colour = Status, shape = Infection)) +
  geom_hline(yintercept = MT.M) + geom_text_repel(
    aes(label = Time), pData(sce.sc)[outliers$MT,], alpha = 0.4) +
  scale_y_continuous(limits = c(0, 100)) + geom_hline(
    yintercept = MT.M + 3 * c(-1,1) * MT.MAD, colour = "red",
    linetype = 2:1
  ) + labs(y = "MT content (% counts)") + sampleQCtheme
```
...
posterDensity <-
  ...
  scale_x_continuous(name = "Overall alignment rate (HISAT2)", limits = c(0,100))
