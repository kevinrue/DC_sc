...
Importantly, *MultiQC* collated metrics for each sample at each step of
...
Let us shorten the sample identifier used by *MultiQC* to a minimal unique identifier (identically to the phenotype information in
...
the [earlier](#featureCounts) use of *MultiQC* reports of read pairs
assignment by *featureCounts*.
...
datatable(
  ...
  options = list(pageLength = 50, searching = TRUE), filter = "top",
  rownames = FALSE
)
```
