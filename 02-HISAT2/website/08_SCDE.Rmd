...
for (time in levels(sce.norm$Time)){
  for (infection in levels(sce.norm$Infection)[-1]){
    for (status in levels(sce.norm$Status)[-1]){
      ...
      DE.res.raw[[contrast]] <- de.raw[de.raw$cZ != 0,]
      DE.res.batch[[contrast]] <- de.batch[de.batch$cZ != 0,]
    }
  }
}
```
...
for (res.name in names(DE.res.batch)){
  ...
  DE.batch.limit[[res.name]] <- de.batch[1:min(1E3,nrow(de.batch)),]
}
```
...
DT::datatable(data.frame(
  "Raw" = sapply(contrastsStimulated, function(n){nrow(DE.res.raw[[n]])}),
  "Batch" = sapply(contrastsStimulated, function(n){nrow(DE.res.batch[[n]])})
  ),
  options = list(pageLength = 12)
)
```
...
for (res.name in names(DE.res.batch)){
  de.batch <- DE.res.batch[[res.name]]
  DE.batch.limit[[res.name]] <- head(de.batch, min(1E3,nrow(de.batch)))
}
```
...
scde.test.gene.expression.difference(
  ...
  groups = groups, batch = batch, n.randomizations = 10)
```
