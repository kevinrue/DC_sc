...
```{r defineTheme, child="_exprsViolinTheme.Rmd", include=FALSE}
```
...
```{r scdeExprDiff, eval=FALSE, warning=FALSE}
DE.res.raw <- list()
...
```{r top10_LT2vsUninfected, fig.height=8}
geneIds <-
...
ggplot(ggLong, aes(Group, norm_exprs)) +
  ...
  theme(
    ...
    strip.text.y = element_text(size = 5)) + exprsViolinTheme
```
...
```{r scdeStimulated, eval=FALSE, warning=FALSE}
contrasts <- list(
