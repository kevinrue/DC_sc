...
for (x_group in levels(sce.pass$Group)){
  ...
  cellIndex <- which(sce.pass$Group == x_group)
...
in each subset of cells against the bulk average,
...
ggplot(bulkSCdata) + geom_abline(slope = 1, intercept = 0, colour = "grey") +
  stat_bin2d(aes(Bulk, Cells), bins = 150) + coord_fixed(ratio = 1) +
  labs(y="Cell average", title="exprs: log2(CPM + offset)") +
  guides(fill = FALSE) + facet_grid(Infection + Status ~ Time) + theme_minimal()
```
...
ggplot(mixC) + geom_point(aes(ERCCmolecules, avgCount)) +
  ...
  labs(
    ...
    y = "Average observed read count in cells passed QC") +
...
```{r density_avgCPM, echo=FALSE, fig.height=8}
