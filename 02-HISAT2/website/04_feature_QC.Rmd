...
```{r PropOver25pct}
propOver10byGroup$PropOver25pct <- (propOver10byGroup$Proportion > 0.50)
```

Let us then define the count of groups in which each gene is detected
above **10** CPM in over **50%** of the group:

```{r groupsOver50pct}
groupsOver50pct <- as.data.frame(with(
    propOver10byGroup,
    tapply(PropOver50pct, Feature, function(x){sum(x)}))
)
colnames(groupsOver50pct) <- "Groups"
groupsOver50pct$Feature <- rownames(groupsOver50pct)
```
...
detectedCPMandProp <- merge(
  subset(propOver10byGroup, Proportion > 0.50), groupsOver50pct, by = "Feature"
)
detectedCPMandProp <- merge(
  detectedCPMandProp,
...
of at least **50%** of cells in each group, while indicating
...
ggplot(detectedCPMandProp) +
  ...
  guides(fill = guide_legend(title = "50% group\n>10 CPM"))
```

```{r detectedCPMandProp_pdf, echo=FALSE}
ggsave("04_out/countFeatures_10CPM_50percent_byGroup.pdf", width = 6, height = 6)
```
