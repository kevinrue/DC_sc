...
```{r volcano_uninfected, echo=FALSE, fig.height=10}
v.data <- do.call(
  "rbind",
  lapply(
...
ggplot(v.data) +
  ...
  ) + theme(strip.text.y = element_text(size = rel(0.5)))
```

## Direct

```{r volcano_direct, echo=FALSE, fig.height=10}
v.data <- do.call(
  "rbind",
  lapply(
...
for (contrastName in grep("^6h", names(scde.res), value = TRUE)){
  ...
  names(de.genes) <- bg.6h; print(table(de.genes))
  pwf <- nullp(de.genes, bias.data = geneLengths[names(de.genes),])
  go.res <- goseq(pwf, "hg38", "ensGene")
  goseq.res[[contrastName]] <- go.res
  saveRDS(goseq.res, "rds/goseq.res_v8.rds") # checkpoint
...
```{r showTopGO}
showTopGO <- function(x, direction = "over", cutoff = 0.01){
  goCols <- c(
  ...
  tmp.go <- head(arrange(tmp.go, over_represented_pvalue), 50)
  DT::datatable(
    tmp.go, options = list(pageLength = 10, searching = TRUE), filter = "top")
}
```
...
```{r add.sig.XY}
add.sig.XY <- function(x){
  sig.levels <- c(NA_character_, "X", "Y", "Both")
  alpha.levels <- c(0.3, 1, 1, 1)
  sig.x <- (x$p.value.x < 0.01); sig.y <- (x$p.value.y < 0.01)
...
for (contrastName in unique(v.data$Contrast)){
  v.subset <- subset(v.data, Contrast == contrastName)
  print(dim(v.subset))
