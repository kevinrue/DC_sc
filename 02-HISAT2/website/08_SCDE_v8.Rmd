...
for (contrastNames in contrasts.2h){
  ...
  names(sg.test) <- sampleNames(sce.ifm.2h); summary(sg.test)
...
ggplot(DE.count, aes(Time, Count, colour = Infection)) +
  ...
  theme_minimal() +
  labs(
    title = "Violet + / Exposed",
    y = "Count of differentially expressed genes"
  )
ggsave("08_out/DE_counts.pdf", width = 7, height = 4)
```
