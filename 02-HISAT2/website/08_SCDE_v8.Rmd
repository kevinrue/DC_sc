...
sub.data <- subset(
  v.data,
  Contrast == "D23580 infected-D23580 exposed" & Time == "4h"
)
...
ggplot(sub.data) +
  ...
  labs(title = "4h STM-D23580", subtitle = "infected - exposed")
...
sub.data <- subset(
  v.data,
  Contrast == "LT2 infected-LT2 exposed" & Time == "4h"
)
geneData <- subset(mutate(
  sub.data,
  gene_name = fData(sce.ifm.4h)[as.character(sub.data$Gene),"gene_name"]),
...
ggplot(sub.data) +
  ...
  scale_x_continuous(limits = mleRange) + theme_bw() +
  labs(title = "4h STM-LT2", subtitle = "infected - exposed")
