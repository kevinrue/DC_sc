...
Prior
DE: {group - all cells w NA; batch - all cells w/out NA}
-->
...
The object `o.ifm` is a `data.frame` with error model coefficients
for each cell:

```{r oIfmTrim, echo=FALSE}
...
The `o.prior` object is a `data.frame` that describes
expression magnitude grid (`x`, on log~10~ scale) and prior (`y`).
...
```{r volcano.mle}
volcano.raw <- function(contrastName){
  scde.adj <- DE.res.raw[[contrastName]]
  ...
  gg <- ggplot(scde.adj, aes(mle, -log10(p.value))) +
    ...
    ggtitle(gsub("_", " ", contrastName), "results")
  print(gg)
  return(scde.adj)
}
volcano.adjusted <- function(contrastName){
  scde.adj <- DE.res.adjusted[[contrastName]]
  scde.adj <- convert.z.score(addGENENAME(orderResults(scde.adj)))
  gg <- ggplot(scde.adj, aes(mle, -log10(p.value))) +
    geom_point(aes(colour = (cZ != 0))) +
    geom_hline(aes(yintercept = -log10(P), linetype = level), volcano.sig) +
    ggtitle(gsub("_", " ", contrastName), "batch.adjusted")
  print(gg)
  return(scde.adj)
}
```
...
```{r volcano.}
tmp <- volcano.raw("6h_D23580_infected-6h_Mock_uninfected")
tmp <- volcano.adjusted("6h_D23580_infected-6h_Mock_uninfected")
```
