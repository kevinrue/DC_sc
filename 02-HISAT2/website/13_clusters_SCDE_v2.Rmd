...
```{r HeatMap_DE_4h, echo=FALSE}
scde.table <- convert.z.score(scde.res[["4h_cluster2-cluster1"]])
fn <- rownames(scde.table)
tmp.gene <- data.frame(
  Gene = fn,
  DE = scde.table[fn, "p.value"] < 0.01,
  Up = scde.table[fn, "mle"] > 0,
  MeanExprs = rowMeans(norm_exprs(sce.ifm.4h)[fn,]),
  p.value = scde.table[fn, "p.value"],
  mle = scde.table[fn, "mle"],
  stringsAsFactors = FALSE
)
scde.genes <- subset(arrange(tmp.gene, Up, -abs(mle)), DE, Gene, drop = TRUE)
scde.cells <-
...
hm <- Heatmap(
  scde.norm,
  name = "norm_exprs", column_title = "4h",
  cluster_columns = FALSE, cluster_rows = FALSE, top_annotation = ha,
  show_row_names = FALSE, show_column_names = FALSE
) +
  Heatmap(
    tmp.gene[scde.genes,"mle"],
    name = "MLE"
  )
draw(hm)
```
...
tmp.gene <- data.frame(
  ...
  DE = scde.table[fn, "p.value"] < 0.01,
  Up = scde.table[fn, "mle"] > 0,
  MeanExprs = rowMeans(norm_exprs(sce.ifm.6h)[fn,]),
  p.value = scde.table[fn, "p.value"],
  mle = scde.table[fn, "mle"],
  stringsAsFactors = FALSE
)
...
hm <- Heatmap(
  scde.norm,
  name = "norm_exprs", column_title = "6h",
  cluster_columns = FALSE, cluster_rows = FALSE, top_annotation = ha,
  show_row_names = FALSE, show_column_names = FALSE
) +
  Heatmap(
