...
scde.table <- data.frame(
  ...
  p.value.cluster = rep(scde.4h.cluster[fn,"p.value"], 2),
  mle.group = c(scde.4h.D23580[fn,"mle"], scde.4h.LT2[fn,"mle"]),
  p.value.group = c(scde.4h.D23580[fn,"p.value"], scde.4h.LT2[fn,"p.value"]),
  Group = c(rep("D23580",length(fn)), rep("LT2",length(fn))),
  GENEID = rep(fn, 2),
  stringsAsFactors = FALSE
)
ggplot(scde.table,aes(-mle.cluster,mle.group,colour=-log10(p.value.group)))+
  geom_point(size = 0.5) + theme_minimal() + facet_grid(~ Group) +
...
scde.table <- subset(scde.table, abs(mle.cluster) > 2.5 & abs(mle.group) > 2.5)
scde.dup.idx <- duplicated(scde.table$GENEID); table(scde.dup)
scde.dup <- scde.table[scde.dup.idx,]
```

Venn diagram:

```{r}
de.4h.D23580 <- rownames(subset(scde.4h.D23580, p.value < 0.01))
de.4h.LT2 <- rownames(subset(scde.4h.LT2, p.value < 0.01))
de.4h.cluster <- rownames(subset(scde.4h.cluster, p.value < 0.01))
vp <- VennDiagram::venn.diagram(
  list(
    D23580 = de.4h.D23580,
    LT2 = de.4h.LT2,
    clusters = de.4h.cluster
  ), NULL
)
