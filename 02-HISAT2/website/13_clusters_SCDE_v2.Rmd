...
ggsave("out_13/cluster_count_2h.pdf", width = 9, height = 4)
```
...
ggsave("out_13/cluster_count_4h.pdf", width = 9, height = 4)
```
...
ggsave("out_13/cluster_count_6h.pdf", width = 9, height = 4)
```
...
ggplot(v.data) +
  ...
  scale_x_continuous(limits = mleRange) + theme_bw() +
  ggtitle("4h: cluster 2 - cluster 1")
ggsave("out_13/4h_volcano_Vogel.pdf", height = 5, width = 6.5)
```
...
ggplot(v.data) +
  ...
  scale_x_continuous(limits = mleRange) + theme_bw() +
  ggtitle("4h: cluster 2 - cluster 1")
ggsave("out_13/4h_volcano_Shalek.pdf", height = 5, width = 6.5)
```
...
ggplot(v.data) +
  ...
  scale_x_continuous(limits = mleRange) + theme_bw() +
  ggtitle("6h: cluster 2 - cluster 1")
ggsave("out_13/6h_volcano_Vogel.pdf", height = 5, width = 6.5)
```
...
ggplot(v.data) +
  ...
  scale_x_continuous(limits = mleRange) + theme_bw() +
  ggtitle("6h: cluster 2 - cluster 1")
ggsave("out_13/6h_volcano_Shalek.pdf", height = 5, width = 6.5)
```
...
ggsave("out_13/4h_correlation_cluster.pdf", width = 6.5, height = 5)
```
...
sampleIdx <- with(pData(sce.ifm.4h), Infection == "D23580" & Time == "4h")
gdata <- data.frame(
...
ggplot(gdata, aes(Status, norm_exprs, fill = Status)) + 
  ...
  scale_colour_manual(values = col.cluster) + theme_minimal()
ggsave(sprintf("out_13/violin_4h_D23580_%s.pdf", geneName),height=4,width=5)
```
...
geneName <- "VPS25"
geneId<-subset(fData(sce.ifm.4h),gene_name==geneName,"gene_id",drop=TRUE)
sampleIdx <- with(pData(sce.ifm.4h), Infection == "LT2" & Time == "4h")
gdata <- data.frame(
  norm_exprs = norm_exprs(sce.ifm.4h)[geneId, sampleIdx],
  pData(sce.ifm.4h)[sampleIdx,c("Infection","Status","quickCluster")],
  row.names = sampleNames(sce.ifm.4h)[sampleIdx]
...
ggplot(gdata, aes(Status, norm_exprs, fill = Status)) + 
  ...
  scale_y_continuous(limits=range(norm_exprs(sce.ifm.4h))) +
  scale_colour_manual(values = col.cluster) + theme_minimal()
ggsave(sprintf("out_13/violin_4h_LT2_%s.pdf", geneName),height=4,width=5)
```
...
geneNames <- c(
  ...
  "DNAJC13","BLOC1S3")
geneIds<-
  subset(fData(sce.ifm.6h), gene_name %in% geneNames, c("gene_id","gene_name"))
geneIds <- geneIds[match(geneNames, geneIds$gene_name),]
...
ht_list <-
  ...
  Heatmap(
    ...
    column_title_gp = gpar(fontsize = 10), column_title = "STM-LT2\ninfected",
    top_annotation = ha.lt2.inf, show_heatmap_legend = FALSE)
pdf("out_13/heatmap_2.pdf", height = 5, width = 7)
draw(ht_list)
dev.off()
```
...
ht_list <-
  ...
  Heatmap(
    ...
    column_title_gp = gpar(fontsize = 10), column_title = "STM-LT2\ninfected",
    top_annotation = ha.lt2.inf, show_heatmap_legend = FALSE)
pdf("out_13/heatmap_3.pdf", height = 5, width = 8)
draw(ht_list)
dev.off()
```
...
stopifnot(nrow(geneIds) == length(geneNames))
norm.mat <-norm_exprs(sce.ifm.6h)[geneIds$gene_id,]
norm.d23 <- norm.mat[,sce.ifm.6h$Infection == "D23580"]
norm.lt2 <- norm.mat[,sce.ifm.6h$Infection == "LT2"]
...
pdf("out_13/heatmap_4.pdf", height = 4, width = 8)
draw(ht_list)
dev.off()
```
