...
ggsave(
  "13_out/cluster_count_2h_wrap.pdf",
  ggplot(pData(sce.ifm.2h)) +
    stat_count(aes(
...
ggsave(
  "13_out/cluster_count_4h_wrap.pdf",
  ggplot(pData(sce.ifm.4h)) +
    stat_count(aes(
      x = interaction(Infection, Status, sep = "\n"),
      fill = quickCluster
    )) +
...
ggsave(
  "13_out/cluster_count_4h_vertical.pdf",
  ggplot(pData(sce.ifm.4h)) +
    stat_count(aes(
      x = interaction(Infection, Status, sep = "\n"),
      fill = quickCluster
...
ggsave(
  "13_out/cluster_count_6h_wrap.pdf",
  ggplot(pData(sce.ifm.6h)) +
    stat_count(aes(
      x = interaction(Infection, Status, sep = "\n"),
      fill = quickCluster
    )) +
...
sg.test <- factor(sce.ifm.4h$quickCluster, levels = c(groupTarget, groupRef))
names(sg.test) <- sampleNames(sce.ifm.4h); summary(sg.test)
...
groupTarget <- 2; groupRef <- 1
sg.test <- factor(sce.ifm.6h$quickCluster, levels = c(groupTarget, groupRef))
names(sg.test) <- sampleNames(sce.ifm.6h); summary(sg.test)
