...
* prepare a set of nine colours used to indicate *cluster membership*,
  *infection*, and *status* information for each sample:
...
normExprsByName.4h <- function(geneId){
  ...
  ggplot(gdata, aes(Status, norm_exprs, fill = Status)) + 
    ...
    scale_y_continuous(limits=range(norm_exprs(sc))) + theme_minimal()
}
...
mleRange2 <-
  max(abs(do.call("c", lapply(
    append(scde.res,list(scde.4h.D23580,scde.4h.LT2,scde.6h.D23580,scde.6h.LT2)),
    function(x){return(x$mle)}))))*c(-1,1)
```
...
scde.table$GENENAME <-
  mapIds(EnsDb.Hsapiens.v79,scde.table$GENEID,"GENENAME","GENEID")
scde.genes <- subset(scde.table, GENENAME %in% c(
  "CTSL","CTSD","IL1B","CD40",
  "CCL1","CCL7","STAT5B","S100A9"))
...
```{r CCL7_normExprsByName.4h, echo=FALSE, warning=FALSE}
normExprsByName.4h("ENSG00000108688")
```

### S100A9

```{r S100A9_normExprsByName.4h, echo=FALSE, warning=FALSE}
normExprsByName.4h("ENSG00000163220")
```

### CTSL

```{r CTSL_normExprsByName.4h, echo=FALSE, warning=FALSE}
normExprsByName.4h("ENSG00000135047")
```

### CTSD

```{r CTSD_normExprsByName.4h, echo=FALSE, warning=FALSE}
normExprsByName.4h("ENSG00000117984")
```

### STAT5B

```{r STAT5B_normExprsByName.4h, echo=FALSE, warning=FALSE}
normExprsByName.4h("ENSG00000173757")
```

### CCL1

```{r CCL1_normExprsByName.4h, echo=FALSE, warning=FALSE}
normExprsByName.4h("ENSG00000108702")
```

### CD40

```{r CD40_normExprsByName.4h, echo=FALSE, warning=FALSE}
normExprsByName.4h("ENSG00000101017")
```

### IL1B

```{r IL1B_normExprsByName.4h, echo=FALSE, warning=FALSE}
...
scde.table$GENENAME <-
  mapIds(EnsDb.Hsapiens.v79,scde.table$GENEID,"GENENAME","GENEID")
scde.genes <- subset(scde.table, GENENAME %in% c(
  "LAMP3","IL12B","IL23A","IL1A",
  "STAT5B","IL6","EBI3","IL36B"))
ggplot(scde.table,aes(-mle.cluster,mle.group))+
  geom_point(size = 0.5, colour = "turquoise3", alpha = 0.2) +
  theme_minimal() + facet_grid(~ Group) +
  scale_x_continuous(limits=mleRange2) + scale_y_continuous(limits=mleRange2) +
...
```{r IL23A_normExprsByName.6h, echo=FALSE, warning=FALSE}
normExprsByName.6h("ENSG00000110944")
```

### IL12B

```{r IL12B_normExprsByName.6h, echo=FALSE, warning=FALSE}
normExprsByName.6h("ENSG00000110944")
```

### EBI3

```{r EBI3_normExprsByName.6h, echo=FALSE, warning=FALSE}
normExprsByName.6h("ENSG00000105246")
```

### IL1A

```{r IL1A_normExprsByName.6h, echo=FALSE, warning=FALSE}
normExprsByName.6h("ENSG00000115008")
```

### IL6

```{r IL6_normExprsByName.6h, echo=FALSE, warning=FALSE}
normExprsByName.6h("ENSG00000136244")
```

### LAMP3

```{r LAMP3_normExprsByName.6h, echo=FALSE, warning=FALSE}
normExprsByName.6h("ENSG00000078081")
```

### STAT5B

```{r STAT5B_normExprsByName.6h, echo=FALSE, warning=FALSE}
normExprsByName.6h("ENSG00000173757")
```

### IL36B

```{r IL36B_normExprsByName.6h, echo=FALSE, warning=FALSE}
...
norm.d23exp <- norm.mat[,sce.ifm.6h$Treatment == "D23580_exposed"]
norm.d23inf <- norm.mat[,sce.ifm.6h$Treatment == "D23580_infected"]
norm.lt2exp <- norm.mat[,sce.ifm.6h$Treatment == "LT2_exposed"]
norm.lt2inf <- norm.mat[,sce.ifm.6h$Treatment == "LT2_infected"]
ha.d23.exp <- HeatmapAnnotation(
  df = pData(sce.ifm.6h)[colnames(norm.d23exp), "quickCluster", drop = FALSE],
  col = list(quickCluster = c("1" = col9[1], "2" = col9[2], "3" = col9[3]))
)
...
scde.cells <-
  sampleNames(sce.ifm.4h)[order(factor(sce.ifm.4h$quickCluster,1:2),na.last=NA)]
...
ha <- HeatmapAnnotation(
  ...
  col = list(
    quickCluster = c("1" = col9[1], "2" = col9[2], "3" = col9[3]),
...
anno.ids <- mapIds(EnsDb.Hsapiens.v79, anno.names, "GENEID", "GENENAME")
...
scde.genes <- rownames(scde.table)[scde.table$p.value < 0.01]
scde.cells <-
  sampleNames(sce.ifm.6h)[order(factor(sce.ifm.6h$quickCluster,1:2),na.last=NA)]
