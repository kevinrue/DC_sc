...
in group `r names(which.min(table(sce.filtered$Group)))`; see this
...
longCluster <- reshape2::melt(
  pData(sce.filtered)[,c("Cluster","Time","Infection","Status")],
  measure.vars = c("Time","Infection","Status"),
  variable.name = "Factor", value.name = "Level"
)
longCluster$Level <- with(
  pData(sce.filtered),
  factor(longCluster$Level, c(levels(Time), levels(Infection), levels(Status)))
)
```

```{r longCluster_plot, echo=FALSE}
ggplot(longCluster) +
  geom_bar(aes(x = interaction(Factor, Level), colour = Factor)) + labs(y = "Cells") +
...
with library size. Let us visualise the two values in a scatter plot:
...
ggplot(
  ...
  facet_grid(Time ~ .) + scale_y_continuous(limits = c(0,2))
```
...
ggplot(pData(sce.filtered), aes(total_counts / 1E6, size_factor_ERCC)) +
  stat_smooth(method = "lm") + geom_point(aes(colour = Time, shape = Status)) +
  theme_minimal() + labs(x = "Library size (millions)", y = "Size factor")
```
