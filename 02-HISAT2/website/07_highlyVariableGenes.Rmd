...
```{r meanVariancePlotOpen, eval=FALSE}
pdf("HVGs/mean-variance.pdf", width = 6, height = 11) # save plots to file
```

```{r HVGs.group, warning=FALSE, message=FALSE, fig.height=6}
par(mfrow=c(5,3)) # figure: grid of 5 rows and 3 columns
HVGs.group <- list()
for (group in levels(sce.norm$Group)){
  ...
  var.out.endo$Symbol <- mapIds(
    EnsDb.Hsapiens.v79, rownames(var.out.endo), "GENENAME", "GENEID")
...
```{r meanVariancePlotClose, eval=FALSE}
dev.off()
```
...
ggplot(hvgsPerGroup) + geom_col(aes(Group, HVGs, alpha = Time, colour = Infection)) +
  theme(
