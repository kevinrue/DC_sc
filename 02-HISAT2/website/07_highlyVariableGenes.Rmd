...
bulk_column <- HeatmapAnnotation(
  ...
  col = list(
    ...
    Infection = c("Mock" = col9[7], "STM-LT2" = col9[8], "STM-D23580" = col9[9])
  )
)
```
...
Alongside single cells:

```{r ht_semisupervised_draw, echo=FALSE}
draw(ht_semisupervised + ht_bulks)
```
...
HVG_LT2_inf <- subset(HVGs.group[["6h_STM-LT2_Violet +"]], mean > 10 & bio > 5)
```
...
write.csv(HVG_LT2_inf, "HVGs/6h_STM-LT2_Violet +_HeHv.csv")
```
...
ggplot(subset(normExprsMelt, gene_name == "CTSB")) +
  geom_density(aes(norm_exprs)) + facet_wrap(~Treatment) + theme_minimal()
```
...
ggplot(normExprsMelt) + facet_wrap(~Treatment) + theme_minimal() +
