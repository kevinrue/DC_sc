...
dm <- model.matrix(~Plate, data = pData(sce.norm))
colnames(dm)
```
...
var.fit.ercc <- trendVar(
  sce.norm, assay = "norm_exprs", method = "loess", parametric = TRUE,
  design = dm, span = 0.4
)
names(var.fit.ercc)
```
...
var.fit.endo <- trendVar(
  sce.norm, assay = "norm_exprs", method = "loess", parametric = TRUE,
  design = dm, span = 0.1, use.spikes = FALSE
)
names(var.fit.endo)
```
...
In addition to the definition above, let us refine HVGs as endogenous features
...
Let us first annotate features with their associated gene name:
...
write.csv(hvg.out, "07_out/hvg.out.csv")
```

For convenience, let us create a copy of the table
...
```{r topBioVariance, results='asis', echo=FALSE}
DT::datatable(
  head(hvg.trim, 100),
...
```{r top.hvg_Infection, echo=FALSE}
plotExpressionSCESet(
...
```{r geneLengths}
geneLengths <- fData(sce.norm)[,"width", drop = FALSE]
```
...
```{r nullp}
pwf <- goseq::nullp(hvgs, bias.data=geneLengths[names(hvgs),], plot.fit=TRUE)
```
...
associated with at least **10** genes in the genome, for robustness:
...
We may then display (in the second tab)
the table restricted to a maximum of **100** most
...
```{r showGO.out.trim}
DT::datatable(
  head(GO.out.trim, 100), # restrict to top 100
  options = list(pageLength = 10, searching = TRUE), filter = "top"
)
```
...
ht_supervised <- Heatmap(
  norm.endo.hvg,
  name = "n.e.", column_title = "Normalised expression",
...
```{r orderBulk}
sce.bulk <- sce.bulk[,with(pData(sce.bulk), order(Time, Status, Infection))]
```
...
bulk_column <- HeatmapAnnotation(
  df = pData(sce.bulk)[,c("Time", "Infection")],
  col = list(
    Time = col.time,
    Infection = col.infection
  )
...
```{r ht_bulks}
ht_bulks <- Heatmap(
  exprs(sce.bulk)[rownames(norm.endo.hvg),],
...
category
[`r GO.out.trim$term[1]`](http://amigo.geneontology.org/amigo/term/`r GO.out.trim$category[1]`)::
...
```{r ht_supervised, fig.height=7, echo=FALSE}
...
for (group in levels(sce.norm$Group)){
  ...
  var.out.endo <- cbind(
    Symbol = with(
      fData(sce.endo), gene_name[match(rownames(var.out.endo),gene_id)]),
    var.out.endo
  )
...
DT::datatable(
  head(GO.HVGs.group[["2h_Mock_Uninfected"]], 100), # restrict to top 100
  options = list(pageLength = 10, searching = TRUE), filter = "top"
)
```
...
DT::datatable(
  head(GO.HVGs.group[["4h_Mock_Uninfected"]], 100), # restrict to top 100
  options = list(pageLength = 10, searching = TRUE), filter = "top"
)
```
...
DT::datatable(
  head(GO.HVGs.group[["6h_Mock_Uninfected"]], 100), # restrict to top 100
  options = list(pageLength = 10, searching = TRUE), filter = "top"
)
```

### 2h_STM-D23580_Exposed

```{r HVGs_2h_D23580_exposed}
DT::datatable(
  head(GO.HVGs.group[["2h_STM-D23580_Exposed"]], 100), # restrict to top 100
  options = list(pageLength = 10, searching = TRUE), filter = "top"
)
```

### 4h_STM-D23580_Exposed

```{r HVGs_4h_D23580_exposed}
DT::datatable(
  head(GO.HVGs.group[["4h_STM-D23580_Exposed"]], 100), # restrict to top 100
  options = list(pageLength = 10, searching = TRUE), filter = "top"
)
```
...
DT::datatable(
  head(GO.HVGs.group[["6h_STM-D23580_Exposed"]], 100), # restrict to top 100
  options = list(pageLength = 10, searching = TRUE), filter = "top"
)
```
...
DT::datatable(
  head(GO.HVGs.group[["2h_STM-LT2_Exposed"]], 100), # restrict to top 100
  options = list(pageLength = 10, searching = TRUE), filter = "top"
)
```
...
DT::datatable(
  head(GO.HVGs.group[["4h_STM-LT2_Exposed"]], 100), # restrict to top 100
  options = list(pageLength = 10, searching = TRUE), filter = "top"
)
```
...
DT::datatable(
  head(GO.HVGs.group[["6h_STM-LT2_Exposed"]], 100), # restrict to top 100
  options = list(pageLength = 10, searching = TRUE), filter = "top"
)
```

### 2h_STM-D23580_Violet +

```{r HVGs_2h_D23580_infected}
DT::datatable(
  head(GO.HVGs.group[["2h_STM-D23580_Violet +"]], 100), # restrict to top 100
  options = list(pageLength = 10, searching = TRUE), filter = "top"
)
```

### 4h_STM-D23580_Violet +

```{r HVGs_4h_D23580_infected}
DT::datatable(
  head(GO.HVGs.group[["4h_STM-D23580_Violet +"]], 100), # restrict to top 100
  options = list(pageLength = 10, searching = TRUE), filter = "top"
)
```

### 6h_STM-D23580_Violet +

```{r HVGs_6h_D23580_infected}
DT::datatable(
  head(GO.HVGs.group[["6h_STM-D23580_Violet +"]], 100), # restrict to top 100
  options = list(pageLength = 10, searching = TRUE), filter = "top"
)
```
...
DT::datatable(
  head(GO.HVGs.group[["2h_STM-LT2_Violet +"]], 100), # restrict to top 100
  options = list(pageLength = 10, searching = TRUE), filter = "top"
)
```
...
DT::datatable(
  head(GO.HVGs.group[["4h_STM-LT2_Violet +"]], 100), # restrict to top 100
  options = list(pageLength = 10, searching = TRUE), filter = "top"
)
```
...
DT::datatable(
  head(GO.HVGs.group[["6h_STM-LT2_Violet +"]], 100), # restrict to top 100
  options = list(pageLength = 10, searching = TRUE), filter = "top"
)
```
...
```{r 6h_D23580_exposed_HeHv}
HeHv_D23_exp <- subset(HVGs.group[["6h_STM-D23580_Exposed"]], mean > 10 & bio > 5)
```

```{r 6h_D23580_exposed_HeHv_datatable, echo=FALSE}
hvg.trim <- HeHv_D23_exp
for (cname in c("mean","total","bio","tech","p.value","FDR")){
  hvg.trim[,cname] <- round(hvg.trim[,cname], 2)
}
DT::datatable(hvg.trim)
```

```{r 6h_D23580_exposed_HeHv.csv, include=FALSE}
write.csv(HeHv_D23_exp, "HVGs/6h_STM-D23580_Exposed_HeHv.csv")
```

### 6h LT2 exposed

```{r 6h_LT2_exposed_HeHv}
HeHv_LT2_exp <- subset(HVGs.group[["6h_STM-LT2_Exposed"]], mean > 10 & bio > 5)
```

```{r 6h_LT2_exposed_HeHv_datatable, echo=FALSE}
hvg.trim <- HeHv_LT2_exp
for (cname in c("mean","total","bio","tech","p.value","FDR")){
  hvg.trim[,cname] <- round(hvg.trim[,cname], 2)
}
DT::datatable(hvg.trim)
...
```{r 6h_LT2_exposed_HeHv.csv, include=FALSE}
write.csv(HeHv_LT2_exp, "HVGs/6h_STM-LT2_Exposed_HeHv.csv")
```

### 6h D23580 Violet +

```{r 6h_D23580_infected_HeHv}
HeHv_D23_inf <- subset(HVGs.group[["6h_STM-D23580_Violet +"]], mean > 10 & bio > 5)
```

```{r 6h_D23580_infected_HeHv_datatable, echo=FALSE}
hvg.trim <- HeHv_D23_inf
for (cname in c("mean","total","bio","tech","p.value","FDR")){
  hvg.trim[,cname] <- round(hvg.trim[,cname], 2)
}
DT::datatable(hvg.trim)
...
```{r 6h_D23580_infected_HeHv.csv, include=FALSE}
write.csv(HeHv_LT2_exp, "HVGs/6h_STM-D23580_Violet +_HeHv.csv")
```

### 6h LT2 Violet +

```{r 6h_LT2_infected_HeHv}
HeHv_LT2_inf <- subset(HVGs.group[["6h_STM-LT2_Violet +"]], mean > 10 & bio > 5)
```

```{r 6h_LT2_infected_HeHv_datatable, echo=FALSE}
hvg.trim <- HeHv_LT2_inf
for (cname in c("mean","total","bio","tech","p.value","FDR")){
  hvg.trim[,cname] <- round(hvg.trim[,cname], 2)
}
DT::datatable(hvg.trim)
...
```{r 6h_LT2_infected_HeHv.csv, include=FALSE}
write.csv(HeHv_LT2_inf, "HVGs/6h_STM-LT2_Violet +_HeHv.csv")
```
...
normExprsMelt <- subset(
  normExprsMelt, Feature %in% unique(c(
    rownames(HeHv_D23_exp),
    rownames(HeHv_LT2_exp),
    rownames(HeHv_D23_inf),
    rownames(HeHv_LT2_inf)))
  )
normExprsMelt <- merge(
...
```{r normExprsMelt_All, echo=FALSE}
ggplot(normExprsMelt) +
  facet_wrap(~Treatment) +
  theme_minimal() +
...
TNFAIP2normExprsMelt <- reshape2::melt(
  norm_exprs(sce.norm)[
    subset(fData(sce.endo), gene_name=="TNFAIP2", "gene_id", drop = TRUE),
...
TNFAIP2normExprsMelt <- merge(
  TNFAIP2normExprsMelt, data.frame(
    Feature = featureNames(sce.norm),
    gene_name = fData(sce.norm)[,"gene_name"])
  )
```
