...
We may then display the table restricted to a maximum of **500**
...
We may now examine the expression data of the endogenous HVGs identified
[above](#identifyHVGs):
...
`r nrow(norm.endo.hvg)` HVGs identified [above](#identifyHVGs):

```{r distEuclidScaled}
hvg.scaled <- t(scale(t(norm.endo.hvg)))
...
```{r distEuclidNorm}
d.e.n <- dist(t(norm.endo.hvg), diag = TRUE, upper = TRUE)
...
```{r distPearsonNorm}
c.p.n <- MKmisc::corDist(t(norm.endo.hvg), diag = TRUE, upper = TRUE)
...
This subset of genes is further investigated [below](#HVGsHE_6h),
as they may represent key genes 
...
For each group of stimulated cells, the [aforementioned](#HVGWithinGroups)
