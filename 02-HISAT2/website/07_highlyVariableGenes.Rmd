...
Tne figure above generally indicates a progressive *up*-regulation of gene
...
In preparation for the heat map, let us define a heat map annotation panel
...
genes.top <- unique(AnnotationDbi::select(
  org.Hs.eg.db, GO.out$category[1], "ENSEMBL", "GOALL")$ENSEMBL
)
table(rownames(hvg.endo) %in% genes.top)
```
...
top_l <- rownames(hvg.endo) %in% genes.top
...
draw(ht.norm + h_top + h_ifn)
```

## Normalised-scaled

```{r ht.scaled, fig.height=7}
draw(ht.scaled + h_top + h_ifn)
```
