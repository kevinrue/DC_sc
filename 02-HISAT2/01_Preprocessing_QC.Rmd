...
ggplot(multiqc) +
  geom_point(aes(
    sample, featureCounts_Assigned, colour = Status, shape = Infection
  )) +
  geom_hline(
...
ggplot(multiqc) +
  geom_point(aes(
    sample, FastQC_percent_gc, colour = Status, shape = Infection
  )) +
  geom_hline(
...
ggplot(multiqc) +
  geom_point(aes(
    sample, Bowtie.2_overall_alignment_rate, colour = Status, shape = Infection
  )) +
  geom_hline(
...
ggplot(multiqc) +
  geom_point(aes(
    sample, featureCounts_percent_assigned, colour = Status, shape = Infection
  )) +
  geom_hline(
...
```{r multiQC_outliers_totalCounts}
metric <- sce.sc$log10_total_counts
multiqc.totalCounts.M <- median(metric)
multiqc.totalCounts.MAD <- mad(metric)
multiqc.totalCounts.outliers <-
  abs(metric - multiqc.totalCounts.M) > 3 * multiqc.totalCounts.MAD
table(multiqc.totalCounts.outliers)
```

In particular, the lower cut-off value is:

```{r totalCounts_lowerCutOff}
10^(multiqc.totalCounts.M - 3 * multiqc.totalCounts.MAD)
```

Let us visualise those outliers, along with blank and bulk samples
for reference:

```{r multiQC_ggplot_totalCounts}
ggplot(pData(sce.all)) +
  geom_point(aes(
    sample, log10_total_counts, colour = Status, shape = Infection
  )) +
  geom_hline(
    yintercept = multiqc.totalCounts.M + 3*c(1,-1)*multiqc.totalCounts.MAD,
    colour = "red"
  ) +
  geom_hline(yintercept = multiqc.totalCounts.M) +
  theme(
    axis.ticks.x = element_blank(), axis.text.x = element_blank(),
    legend.position = "bottom", legend.box = "vertical"
  )
...
ggplot(pData(sce.all)) +
  geom_point(aes(
    sample, pct_counts_top_50_features, colour = Status, shape = Infection
  )) +
  geom_hline(
...
ggplot(pData(sce.all)) +
  geom_point(aes(
    sample, pct_counts_top_100_features, colour = Status, shape = Infection
  )) +
  geom_hline(
...
ggplot(pData(sce.all)) +
  geom_point(aes(
    sample, pct_counts_top_200_features, colour = Status, shape = Infection
  )) +
  geom_hline(
...
ggplot(pData(sce.all)) +
  geom_point(aes(
    sample, total_features, colour = Status, shape = Infection
  )) +
  geom_hline(
...
ggplot(pData(sce.all)) +
  geom_point(aes(
      sample, pct_counts_feature_controls_MT, colour = Status, shape = Infection
  )) +
  geom_hline(
