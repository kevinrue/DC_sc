...
Excessively small library size may indicate technical issues during
...
### Percentage assigned (_featureCounts_)
...
**Note:** The above command skips records relating to other features present
in the GTF, such as `exon` (feature; third column) and `ERCC` (source; second
column).
...
dm <- model.matrix(~Plate+Lane, data = pData(sce.filtered))
colnames(dm)
```
...
var.fit.ercc <- trendVar(
  sce.filtered, assay = "norm_exprs", trend = "loess", design = dm,
  span = 0.4
)
names(var.fit.ercc)
```
...
```{r decomposeVar_ercc}
var.out.ercc <- decomposeVar(sce.filtered, var.fit.ercc, assay = "norm_exprs")
names(var.out.ercc)
```
...
`r Biocpkg("scran")` `trendVar` method to estimate technical variation under
the assumption stated above, accouting for the same technical factors of the
experimental design:
...
```{r decomposeVar_endo}
var.out.endo <- decomposeVar(sce.filtered, var.fit.endo)
names(var.out.endo)
```
...
spike.fit <- trendVar(sce.filtered, use.spikes=TRUE) # To compute spike-in variances.
...
```{r annotated_hvg.out}
hvg.out <- cbind(
  gene_name = gtfData$gene_name[match(rownames(hvg.out), gtfData$gene_id)],
  hvg.out
)
```
...
top.hvg <- rownames(hvg.out)[11:20]
top.hvg <- rownames(hvg.out)[21:30]
top.hvg <- rownames(hvg.out)[31:40]
plotExpressionSCESet(
  sce.filtered, top.hvg, exprs_values = "norm_exprs", colour_by = "Time"
) +
  scale_x_discrete(labels = fData(sce.filtered[top.hvg,])$gene_name)
```
...
```{r top.hvg_Infection}
plotExpressionSCESet(
  sce.filtered, top.hvg, exprs_values = "norm_exprs", colour_by = "Infection"
) +
  scale_x_discrete(labels = fData(sce.filtered[top.hvg,])$gene_name)
```
...
```{r top.hvg_Status}
plotExpressionSCESet(
  sce.filtered, top.hvg, exprs_values = "norm_exprs", colour_by = "Status"
) +
  scale_x_discrete(labels = fData(sce.filtered[top.hvg,])$gene_name)
```
