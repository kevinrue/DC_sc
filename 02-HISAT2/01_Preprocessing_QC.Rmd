...
author: "Anna Aulicino, Kévin Rue-Albrecht"
date: "`r doc_date()`"
...
sce.pass <- calculateQCMetrics(
  sce.pass, feature_controls = list(ERCC = ERCCs, MT = MTs$GENEID)
)
```
...
Let us set define low-abundance genes as those with an average count below a
...
Let us apply the unsupervised (*i.e.*, unaware of experimental phenotypes)
normalisation methodology implemented in the `r Biocpkg("scran")`
package.

The first step of the procedure is to pool counts from many cells for accurate
size factor estimation (**cite Lun et al 2016**), before deconvolution into
cell-based factors for cell-specific normalization
(**cite paper 67 from Davis**):

```{r computeSumFactors}
sce.filtered <- computeSumFactors(sce.filtered, sizes = c(20, 40, 60, 80))
...
more important in `uninfected` cells, and at the ealiest time point
...
Having normalised the expression 

Larger experiments contain more technical factors that need to be investigated. In this dataset, factors include the sex
of the animal from which the cells were extracted, the age of the animal, the tissue of origin for each cell, and the total
spike-in count in each cell. Figure 19 shows that the tissue of origin explains a substantial proportion of the variance
for a subset of genes. This is probably because each tissue contains a different composition of cell types, leading to
systematic differences in gene expression between tissues. The other factors explain only a small proportion of the
variance for most genes and do not need to be incorporated into our downstream analyses.
...
Having selected single cells that pass all quality control criteria in the
previous sections, let us examine the unsupervised clustering of those cells
to validate the absence of extreme outliers:

```{r plotPCASCESet_status}
plotPCASCESet(
  sce.filtered, ncomponents = 3,
  colour_by = "Status", shape_by = "Time",
  feature_set = !fData(sce.filtered)$is_feature_control_ERCC
)
```

**Note 1:** Some important default values are:

* `exprs_values="exprs"`: the log~2~-transformed CPM (after raw counts were
  offset by `1` count) are used.
...
* `scale_features=FALSE`: to use directly the log~2~-transformed CPM values
  as between-sample normalisation was not performed yet.
