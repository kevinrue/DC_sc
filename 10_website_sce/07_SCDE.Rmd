...
for (contrastNames in contrasts.2h){
  ...
  saveRDS(scde.res, "rds/scde_group.rds") # checkpoint
}
```
...
```{r Vogel_volcano_6h_D23LT2_Violet, include=FALSE}
z.range <- max(abs(v.data$Z)) * c(-1,1)
p.range <- rev(-log10(range(v.data$p.value)))
sub.data <- subset(
  v.data,
  Contrast == "STM-D23580 Violet +\nSTM-LT2 Violet +" & Time == "6h"
)
geneData <- subset(dplyr::mutate(
  sub.data,
...
ggplot(sub.data) +
  ...
  theme_bw() +
  labs(
    title = "6h Violet +", subtitle = "STM-D23580 / STM-LT2",
    y = expression(-log[10]*" ("*italic(P)*"-value)")
  )
ggsave(file.path(outdir, "6h_volcano_Violet +_Vogel.pdf"), height=5, width=6.5)
```
