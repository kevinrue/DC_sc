...
stopifnot(all(rownames(RG) == names(gtf.final)))
```
...
sce.all <- SingleCellExperiment(
  assays=list(counts=RG$counts), colData=pd.all, rowRanges=rowranges)
dim(sce.all)
```
...
sce.blank <- SingleCellExperiment(
  ...
  colData=pd.blank, rowRanges=rowranges)
dim(sce.blank)
```
...
sce.bulk <- SingleCellExperiment(
  ...
  colData=pd.bulk, rowRanges=rowranges)
dim(sce.bulk)
```
...
sce.sc <- SingleCellExperiment(
  ...
  colData=pd.sc, rowRanges=rowranges)
dim(sce.sc)
```
