...
# Heat map of top 50 markers fold-change {.tabset}

For each time point, let us display up to 50 marker genes for each cluster,
selected for having:

* *P*-value less than 0.01
* *mle* of log fold-change among the 50 largest positive value for a cluster
...
rownames(markersMatrix) <- scde.table[rownames(markersMatrix), "GENENAME"]
ht <- Heatmap(
...
# Heat map of top 50 markers log-counts {.tabset}
...
ht_list <- list()
for (clusterId in levels(sce.ifm.2h$quickCluster.2h)){
  markersMatrix <- assay(sce.ifm.2h, "logcounts")[
    markers_2h, sce.ifm.2h$quickCluster.2h == clusterId
  ]
  rownames(markersMatrix) <- scde.table[rownames(markersMatrix), "GENENAME"]
...
ht_list <- list()
for (clusterId in levels(sce.ifm.4h$quickCluster.4h)){
  markersMatrix <- assay(sce.ifm.4h, "logcounts")[
    markers_4h, sce.ifm.4h$quickCluster.4h == clusterId
  ]
  rownames(markersMatrix) <- scde.table[rownames(markersMatrix), "GENENAME"]
  ht_list[[clusterId]] <- Heatmap(
    matrix = markersMatrix,
...
ht_list <- list()
for (clusterId in levels(sce.ifm.6h$quickCluster.6h)){
  markersMatrix <- assay(sce.ifm.6h, "logcounts")[
    markers_6h, sce.ifm.6h$quickCluster.6h == clusterId
  ]
  rownames(markersMatrix) <- scde.table[rownames(markersMatrix), "GENENAME"]
  ht_list[[clusterId]] <- Heatmap(
    matrix = markersMatrix,
