...
var.fit.ercc <- trendVar(
  sce.norm, assay.type="logcounts", method = "loess", parametric = TRUE,
  design = dm, span = 0.4, use.spikes=TRUE
)
names(var.fit.ercc)
```
...
```{r plot_var.out.ercc}
with(var.out.ercc, plot(
  mean, total, pch = 16, cex = 0.6,
  xlab = "Mean log-expression", ylab = "Variance of log-expression"
))
with(var.fit.ercc, points(
  mean, var, col="red", pch=16
))
points(var.fit.ercc$mean, var.fit.ercc$var, col="red", pch=16)
o <- order(var.out.ercc$mean);
with(var.out.ercc, lines(mean[o], tech[o], col="red", lwd=2))
```
...
var.fit.endo <- trendVar(
  ...
  design = dm, span = 0.1, assay.type="logcounts", use.spikes=FALSE
)
names(var.fit.endo)
```
...
```{r plot_var.out.endo}
with(var.out.endo, plot(
  mean, total, pch=16, cex=0.6,
  xlab="Mean log-expression", ylab="Variance of log-expression"
))
o <- order(var.out.endo$mean)
with(var.out.endo, lines(
  mean[o], tech[o], col="dodgerblue", lwd=2))
spike.fit <- trendVar(sce.norm, assay.type="logcounts", use.spikes=TRUE) # To compute spike-in variances.
with(spike.fit, points(mean, var, col="red", pch=16))
```
...
```{r geneLengths}
geneLengths <- width(rowRanges(sce.norm[hvgs,]))
```
...
for (group in levels(sce.norm$Group)){
  ...
  with(
    var.out.endo,
    plot(
      mean, total, pch=16, cex=0.2,
      xlab="Mean log-expression", ylab="Variance of log-expression",
      main = group
    )
  )
  o <- order(var.out.endo$mean)
  with(var.out.endo, lines(mean[o], tech[o], col="dodgerblue", lwd=2))
  spike.fit <- trendVar(
    sce.norm[,idx.group], assay.type="logcounts", use.spikes=TRUE)
  with(spike.fit, points(mean, var, col="red", pch=16, cex=0.4))
  var.out.endo <- cbind(
    Symbol = with(
...
saveRDS(HVGs.group, file.path(outdir, "HVGs_Group.rds"))
```
...
ggplot(TNFAIP2normExprsMelt) +
  geom_density(aes(logcounts, colour=Time)) +
...
h_column <- HeatmapAnnotation(
  df = as.data.frame(colData(sce.endo.4hStim)[,c("Status", "Infection")]),
  col = list(
    Status = col.status,
    Infection = col.infection
  )
