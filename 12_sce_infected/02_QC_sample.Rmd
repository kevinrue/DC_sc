...
outdir <- "02_out"; dir.create(outdir, showWarnings = FALSE)
sce.all$GroupLabel <- with(colData(sce.all), factor(
  gsub("_", " ", Group),
  gsub("_", " ", levels(Group))
))
sce.sc$GroupLabel <- with(colData(sce.sc), factor(
  gsub("_", " ", Group),
  gsub("_", " ", levels(Group))
))
sce.blank$GroupLabel <- with(colData(sce.blank), factor(
  gsub("_", " ", Group),
  gsub("_", " ", levels(Group))
))
```
...
names(col.infection) <- levels(sce.all$Infection)
...
sce.blank <- calculateQCMetrics(
  sce.blank,
  exprs_values = "counts",
  feature_controls = list(ERCC = isSpike(sce.blank, "ERCC"))
)
```
...
ggsave(
  ...
  plot.GC + guides(colour = "none", shape = "none"), width = 8, height = 4.25
)
plot.GC <-
  ggplot(
...
ggsave(
  ...
  plot.aligned + guides(colour = "none", shape = "none"), width = 8, height = 4.25
)
plot.aligned <-
  ggplot(
    data.frame(colData(sce.all)),
...
ggsave(
  ...
  plot.cAssigned + guides(colour = "none", shape = "none"), width = 8, height = 4.25
)
plot.cAssigned <-
  ggplot(
    data.frame(colData(sce.all)),
...
ggsave(
  ...
  plot.pAssigned + guides(colour = "none", shape = "none"), width = 8, height = 4.25
)
plot.pAssigned <-
  ggplot(
    data.frame(colData(sce.all)),
...
plot.pAssigned
ggsave(
  file.path(outdir, "assigned_rate_group.pdf"),
  plot.pAssigned + guides(colour = "none", shape = "none"), width = 8, height = 4.25
)
```
...
sce.sc <- calculateQCMetrics(
  ...
  feature_controls = list(
    ERCC = isSpike(sce.sc, "ERCC"),
    MT = as.logical(seqnames(sce.sc) == "MT")
  )
)
```
...
plot.top50 <-
  ggplot(
    data.frame(colData(sce.all)),
    aes(Sample, pct_counts_top_50_features)) +
  geom_hline(
...
ggsave(
  ...
  plot.top50 + guides(colour = "none", shape = "none"), width = 8, height = 4.25
)
plot.top50 <-
  ggplot(
    data.frame(colData(sce.all)),
...
ggsave(
  ...
  plot.top100 + guides(colour = "none", shape = "none"), width = 8, height = 4.25
)
plot.top100 <-
  ggplot(
    data.frame(colData(sce.all)),
...
ggsave(
  ...
  plot.top200 + guides(colour = "none", shape = "none"), width = 8, height = 4.25
)
plot.top200 <-
  ggplot(
    data.frame(colData(sce.all)),
...
ggsave(
  ...
  plot.features + guides(colour = "none", shape = "none"), width = 8, height = 4.25
)
plot.features <-
  ggplot(
    data.frame(colData(sce.all)),
...
pander::pandoc.table(
  data.frame(
    ...
    Failed = as.numeric(table(sce.sc$Group)) - 
      as.numeric(table(sce.pass$Group)),
    Time = factor(
      gsub("(.*)_.*_.*", "\\1", levels(sce.pass$Group)),
      levels(sce.pass$Time)
    ),
    Infection = factor(
      gsub(".*_(.*)_.*", "\\1", levels(sce.pass$Group)),
      levels(sce.pass$Infection)
    ),
    Status = factor(
      gsub(".*_.*_(.*)", "\\1", levels(sce.pass$Group)),
      levels(sce.pass$Status)
    )
  ),
...
ggsave(
  file.path(outdir, "poster_densityLibrary.pdf"), posterDensity,
  width=8, height = 6)
```
