...
map <- getBM(c("external_gene_name", "go_id"), mart=mart)
```
...
for (groupName in names(geneGroups)) {
    ...
    pvals$geneInGO <- ""
    pvals$geneDE <- ""
    significantGO <- subset(pvals, over_represented_pvalue < 0.001, "category", drop=TRUE)
    message("significantGO: ", length(significantGO))
    for (go_id in head(significantGO, 100)) {
        message("go_id: ", go_id)
        out <- getBM(c("ensembl_gene_id", "external_gene_name", "go_id"), filters = "go", values = go_id, mart=mart)
        genesInGO <- unique(out$ensembl_gene_id)
        message("genesInGO: ", length(genesInGO))
        genesDE <- unique(intersect(genesInGO, names(genes)[genes]))
        geneNamesDE <- unique(subset(out, ensembl_gene_id %in% genesDE, "external_gene_name", drop=TRUE))
        message("genesDE: ", length(genesDE))
        indexGO <- match(go_id, pvals$category)
        pvals$geneInGO[indexGO] <- paste(unique(out$external_gene_name), collapse = ",")
        pvals$geneDE[indexGO] <- paste(geneNamesDE, collapse = ",")
    }
