...
outdir <- "09_markers_20180708_out"; dir.create(outdir, showWarnings = FALSE)
...
for (contrastName in c2h){
    ...
    scde.markers <- head(rownames(scde.table), 30)
...
for (clusterId in unique(markers_2h_FC.cluster_no0)){
    ...
    ha_tmp <- HeatmapAnnotation(
        df = data.frame(subset(
            colData(sce.ifm.2h), quickCluster.2h == clusterId,
            c("quickCluster.2h", "Infection", "Status")
        )),
        col = list(
            quickCluster.2h = col.cluster,
            Infection = col.infection,
            Status = col.status
        )
    )
...
for (clusterId in levels(sce.ifm.6h$quickCluster.6h)){
    ...
    ht_list[[clusterId]] <- Heatmap(
        ...
        row_names_side = "left",
        top_annotation = ha_tmp,
        split = markers_6h_FC.cluster, gap = unit(5, "mm")
    )
}
ht_final <- ht_list[["0"]] + ht_list[["1"]] + ht_list[["2"]] + ht_list[["3"]]
draw(ht_final)
...
# Top 30 markers by significance
...
for (contrastName in c2h){
    ...
    scde.markers <- head(rownames(scde.table), 30)
    #
    markersMatrix <- assay(sce.ifm.2h, "logcounts")[scde.markers,]
    markersMatrix <- t(scale(t(markersMatrix)))
    markersDist <- dist(markersMatrix)
    markersClust <- hclust(markersDist)
    co <- order.optimal(markersDist, markersClust$merge)
    markersClust$merge <- co$merge
    markersClust$order <- co$order
    scde.markers <- scde.markers[markersClust$order]
    #
...
for (contrastName in c4h){
    ...
    scde.markers <- head(rownames(scde.table), 30)
    #
    markersMatrix <- assay(sce.ifm.4h, "logcounts")[scde.markers,]
    markersMatrix <- t(scale(t(markersMatrix)))
    markersDist <- dist(markersMatrix)
    markersClust <- hclust(markersDist)
    co <- order.optimal(markersDist, markersClust$merge)
...
for (contrastName in c6h){
    ...
    scde.markers <- head(rownames(scde.table), 30)
    #
    markersMatrix <- assay(sce.ifm.6h, "logcounts")[scde.markers,]
    markersMatrix <- t(scale(t(markersMatrix)))
    markersDist <- dist(markersMatrix)
    markersClust <- hclust(markersDist)
    co <- order.optimal(markersDist, markersClust$merge)
...
## Heat map of fold-change for (up to) 30 markers {.tabset}
...
## Heat map of row-scaled log-counts for (up to) 30 markers {.tabset}
...
for (clusterId in levels(sce.ifm.2h$quickCluster.2h)){
    ...
    ht_list[[clusterId]] <- Heatmap(
        ...
        show_heatmap_legend =
            clusterId == levels(sce.ifm.2h$quickCluster.2h)[1],
        show_row_names = clusterId == tail(levels(sce.ifm.2h$quickCluster.2h), 1),
        cluster_rows = FALSE,
        show_column_names = FALSE, show_column_dend = FALSE,
        row_names_side = "right",
        top_annotation = ha_tmp,
        split = markers_2h_p.cluster, gap = unit(5, "mm")
    )
}
ht_final <- ht_list[["0"]] + ht_list[["1"]] + ht_list[["2"]] + ht_list[["3"]]
draw(ht_final)
...
pdf(
    file.path(outdir, "2h_markers_P_heatmap_logcounts_10x18.pdf"),
    width = 10, height = 18)
draw(ht_final)
dev.off()
```
...
for (clusterId in unique(markers_2h_p.cluster_no0)){
    ...
    ht_list[[clusterId]] <- Heatmap(
        ...
        show_heatmap_legend = clusterId == "3",
        show_row_names = clusterId == "3",
        cluster_rows = FALSE,
        show_column_names = FALSE, show_column_dend = FALSE,
        row_names_side = "right",
...
for (clusterId in levels(sce.ifm.4h$quickCluster.4h)){
    ...
    ha_tmp <- HeatmapAnnotation(
        df = data.frame(subset(
            colData(sce.ifm.4h), quickCluster.4h == clusterId,
            c("quickCluster.4h", "Infection", "Status")
        )),
        col = list(
            Infection = col.infection,
            Status = col.status,
            quickCluster.4h = col.cluster
        )
    )
    ht_list[[clusterId]] <- Heatmap(
        matrix = clusterMatrix,
        ...
        show_heatmap_legend =
            clusterId == levels(sce.ifm.4h$quickCluster.4h)[1],
        show_row_names = clusterId == tail(levels(sce.ifm.4h$quickCluster.4h), 1),
        cluster_rows = FALSE,
        show_column_names = FALSE, show_column_dend = FALSE,
        row_names_side = "right",
        top_annotation = ha_tmp,
        split = markers_4h_p.cluster, gap = unit(5, "mm")
    )
}
ht_final <- ht_list[["0"]] + ht_list[["1"]] + ht_list[["2"]] + ht_list[["3"]]
draw(ht_final)
...
pdf(
    file.path(outdir, "4h_markers_P_heatmap_logcounts_10x18.pdf"),
    width = 10, height = 18)
draw(ht_final)
dev.off()
```
...
for (clusterId in levels(sce.ifm.6h$quickCluster.6h)){
    ...
    ha_tmp <- HeatmapAnnotation(
        df = data.frame(subset(
            colData(sce.ifm.6h), quickCluster.6h == clusterId,
            c("quickCluster.6h", "Infection", "Status")
        )),
        col = list(
            Infection = col.infection,
            Status = col.status,
            quickCluster.6h = col.cluster
        )
    )
    ht_list[[clusterId]] <- Heatmap(
        matrix = clusterMatrix,
        ...
        show_heatmap_legend =
            clusterId == levels(sce.ifm.6h$quickCluster.6h)[1],
        show_row_names = clusterId == tail(levels(sce.ifm.6h$quickCluster.6h), 1),
        cluster_rows = FALSE,
        show_column_names = FALSE, show_column_dend = FALSE,
        row_names_side = "right",
        top_annotation = ha_tmp,
        split = markers_6h_p.cluster, gap = unit(5, "mm")
    )
}
ht_final <- ht_list[["0"]] + ht_list[["1"]] + ht_list[["2"]] + ht_list[["3"]]
draw(ht_final)
...
pdf(
    file.path(outdir, "6h_markers_P_heatmap_logcounts_10x18.pdf"),
    width = 10, height = 18)
draw(ht_final)
dev.off()
```
