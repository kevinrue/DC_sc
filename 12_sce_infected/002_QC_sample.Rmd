...
Let us identify as outliers minibulks that display a GC content
excessively *different* from the median of all minibulks,
here beyond **3** MADs:
...
let us identify as outliers minibulks with an alignment rate
excessively *lower* than the median of all minibulks ,
here beyond **3** MADs:
...
Let us identify as outliers minibulks
...
Let us display the count of minibulks identified as outliers for each of the
filters so far:
...
of minibulks before any filtering;
conversely, the last row indicates the count of minibulks that failed or
...
leverage control features, as means to identify outliers among minibulks
...
In the `SingleCellExperiment` object augmented by QC metrics,
...
outliers$top50 <- isOutlier(sce.mini$pct_counts_in_top_50_features, 3)
...
outliers$top100 <- isOutlier(sce.mini$pct_counts_in_top_50_features, 3)
...
outliers$top200 <- isOutlier(sce.mini$pct_counts_in_top_50_features, 3)
...
top50.M <- median(sce.mini$pct_counts_in_top_50_features)
top50.MAD <- mad(sce.mini$pct_counts_in_top_50_features)
plot.top50 <-
  ggplot(data.frame(colData(sce.mini)), aes(Sample, pct_counts_in_top_50_features)) +
  geom_hline(
...
plot.top50 <-
  ggplot(
    data.frame(colData(sce.mini)),
    aes(GroupLabel, pct_counts_in_top_50_features)) +
  geom_violin(aes(colour = Infection), alpha = 0.25) +
  geom_hline(
    yintercept = top50.M + 3*c(-1,1)*top50.MAD,
    colour = "red", linetype = 2:1, alpha = 0.5) +
...
top100.M <- median(sce.mini$pct_counts_in_top_100_features)
top100.MAD <- mad(sce.mini$pct_counts_in_top_100_features)
plot.top100 <-
  ggplot(data.frame(colData(sce.mini)), aes(Sample, pct_counts_in_top_100_features)) +
  geom_hline(
...
plot.top100 <-
  ggplot(
    data.frame(colData(sce.mini)),
    aes(GroupLabel, pct_counts_in_top_100_features)) +
  geom_violin(aes(colour = Infection), alpha = 0.25) +
  geom_hline(
    yintercept = top100.M + 3*c(-1,1)*top100.MAD,
    colour = "red", linetype = 2:1, alpha = 0.5) +
...
top200.M <- median(sce.mini$pct_counts_in_top_200_features)
top200.MAD <- mad(sce.mini$pct_counts_in_top_200_features)
plot.top200 <-
  ggplot(data.frame(colData(sce.mini)), aes(Sample, pct_counts_in_top_200_features)) +
  geom_hline(
...
plot.top200 <-
  ggplot(
    data.frame(colData(sce.mini)),
    aes(GroupLabel, pct_counts_in_top_200_features)) +
  geom_violin(aes(colour = Infection), alpha = 0.25) +
  geom_hline(
    yintercept = top200.M + 3*c(-1,1)*top200.MAD,
    colour = "red", linetype = 2:1, alpha = 0.5) +
...
outliers$FeaturesCount <- isOutlier(sce.mini$total_features_by_counts, 2, "lower")
...
features.M <- median(sce.mini$total_features_by_counts)
features.MAD <- mad(sce.mini$total_features_by_counts)
plot.features <-
  ggplot(data.frame(colData(sce.mini)), aes(Sample, total_features_by_counts)) +
  ...
  geom_text_repel(
    ...
    alpha = 0.6, min.segment.length = unit(0, "lines")) +
  scale_colour_manual(values = col.infection) +
  scale_y_continuous(limits=c(0,max(sce.mini$total_features_by_counts))*1.1) +
...
plot.features <-
  ggplot(
    data.frame(colData(sce.mini)),
    aes(GroupLabel, total_features_by_counts)) +
  geom_violin(aes(colour = Infection), alpha=0.25) +
  geom_hline(yintercept = features.M, alpha = 0.5) +
  geom_hline(
    yintercept = features.M + 2 * c(-1,1) * features.MAD, colour = "red",
...
hm <- Heatmap(
  ...
  col = c("Pass" = "lightblue", "Fail" = "red"),
  cluster_rows = FALSE, show_row_names = FALSE, cluster_columns = FALSE,
  split = colData(sce.mini)[cells.order,c("Donor")],
  combined_name_fun = function(x) paste(x, collapse = "\n"),
  row_title_gp = gpar(fontsize = 7)
)
pdf(file.path(outdir, "Heatmap_QC_donor.pdf"), height = 8, width = 4)
draw(hm + hm_row)
dev.off()
```
...
pander::pandoc.table(
  data.frame(
    Initial = as.numeric(table(sce.mini$Group)),
    Passed = as.numeric(table(sce.excludeD3$Group)),
    Failed = as.numeric(table(sce.mini$Group)) - 
      as.numeric(table(sce.excludeD3$Group)),
    Time = factor(
      gsub("(.*)_.*_.*", "\\1", levels(sce.excludeD3$Group)),
      levels(sce.excludeD3$Time)
    ),
    Infection = factor(
      gsub(".*_(.*)_.*", "\\1", levels(sce.excludeD3$Group)),
      levels(sce.excludeD3$Infection)
    ),
    Status = factor(
      gsub(".*_.*_(.*)", "\\1", levels(sce.excludeD3$Group)),
      levels(sce.excludeD3$Status)
    )
  ),
